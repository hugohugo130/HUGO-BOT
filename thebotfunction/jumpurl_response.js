...
module.exports = {
    setup(client) {
        try {
            ...
            console.error(`[${time()}] 處理跳轉URL時出錯：`, error);
            const { chatting_channel_ID, HugoUserID } = require("../config.json");
            client.channels.cache.get(chatting_channel_ID).send(`[${time()}] 處理跳轉URL時出錯：${error}\n<@${HugoUserID}>`);
            client.users.send(HugoUserID, `[${time()}] 處理跳轉URL時出錯：${error}`);
        };
    },
};