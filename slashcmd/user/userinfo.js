...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let isadminpermission = member.permissions.has({ checkAdmin: true });
        ...
        let embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "暱稱", value: displayname ? displayname : "無" },
                { name: "用戶id", value: id },
                { name: "最高身份組", value: toprole.name },
                { name: "具有伺服器管理員權限", value: isadminpermission ? "是" : "否" },
                { name: "機器人管理員", value: isbotadmin ? "是" : "否" },
                { name: "賬號創建於", value: `<t:${createdts}> <t:${createdts}:T> (<t:${createdts}:R>)` },
                { name: `加入 ${guild.name} 於`, value: `<t:${joinedts}> <t:${joinedts}:T> (<t:${joinedts}:R>)` },
                { name: "機器人", value: bot ? "是" : "否" },
            );
